...
public sealed class MutagenController : ISyncSessionController, IAsyncDisposable
{
    ...
    private readonly string _mutagenExecutablePath = Path.Combine(AppContext.BaseDirectory, "mutagen.exe");
    ...
    public async Task Initialize(CancellationToken ct)
    {
        ...
        using (_ = await _lock.LockAsync(ct))
        {
            ...
            _ = transition.ContinueWith(RemoveTransition, cancellationToken: CancellationToken.None);
        ...
        while (true)
        {
            ...
            using (_ = await _lock.LockAsync(ct))
            {
                ...
                else
                {
                    ...
                    transition = _inProgressTransition;
                    _ = transition.ContinueWith(RemoveTransition, cancellationToken: ct);
                }
            }
    ...
    private async Task<MutagenClient?> WaitForDaemon(CancellationToken ct)
    {
        while (true)
        {
            ...
            catch (Exception e) when (e is not OperationCanceledException) // TODO: Are there other permanent errors we can detect?
            {
...
public class LogWriter(StreamReader reader, StreamWriter writer) : IDisposable
{
    ...
    public async Task Run()
    {
        ...
        {
            await Console.Out.WriteLineAsync("caught LogWriter exception");
        }
        finally
        {
            Dispose();
